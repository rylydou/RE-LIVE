...
namespace RELIVE.Stage
{
	...
	public struct StageData
	{
		...
		public void UnpackData(bool bCleanupData = false)
		{
			...
			foreach (var element in elements) element.UnpackData(bCleanupData);

			if (bCleanupData) serializableTiles = null;
		}

		public void PackData(bool bCleanupData = false)
		{
			serializableTiles = new int[size.x * size.y];
			...
			foreach (var element in elements) element.PackData(bCleanupData);

			if (bCleanupData) tiles = null;
		}
		#endregion
